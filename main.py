...
class AnketaModalPart2(Modal, title="Анкета — часть 2 из 2"):
    ...
    activities = TextInput(label="7. Чем планируете заниматься на сервере?", style=discord.TextStyle.paragraph, placeholder="PvP, PvE, строительство, торговля и т.д.", required=True, max_length=300)
    previous_guilds = TextInput(label="9. Поселения / города / гильдии на этом сервере", style=discord.TextStyle.short, placeholder="Если не было — напишите «нет»", required=True, max_length=300)
    other_projects = TextInput(label="10. Опыт в других похожих проектах", style=discord.TextStyle.paragraph, placeholder="Перечислите проекты и ваш опыт там", required=True, max_length=400)
    pvp_skill = TextInput(label="11. Оцените PvP-скиллы (1–10)", style=discord.TextStyle.short, placeholder="Введите целое число от 1 до 10", required=True, max_length=2)
    mechanics_skill = TextInput(label="12. Знание механик сервера (1–10)", style=discord.TextStyle.short, placeholder="Введите целое число от 1 до 10", required=True, max_length=2)
...
class AnketaModalPart1(Modal, title="Анкета — часть 1 из 2"):
    ...
    hours_per_day = TextInput(label="3. Сколько часов в день готовы уделять серверу?", style=discord.TextStyle.short, placeholder="Например: 3–5 часов", required=True, max_length=50)
    bans = TextInput(label="4. Были ли баны на проекте? Если да — за что?", style=discord.TextStyle.paragraph, placeholder="Если не было — напишите «нет»", required=True, max_length=500)
    season1 = TextInput(label="5. Играли ли на 1 сезоне? Ваши достижения", style=discord.TextStyle.paragraph, placeholder="Если не играли — напишите «нет»", required=True, max_length=500)
...
class MicrophoneView(View):
    def __init__(self, data: dict, user: discord.User):
        ...
        team_select = Select(placeholder="13. Готовы играть в команде и следовать целям?", options=[discord.SelectOption(label="✅ Да", value="да", emoji="🤝"), discord.SelectOption(label="❌ Нет", value="нет", emoji="🚫")], custom_id="team_select", row=1)
    ...
    async def submit_callback(self, interaction: discord.Interaction):
        if self.mic_value is None or self.team_value is None:
            await interaction.response.send_message("⚠️ Пожалуйста, выберите ответы на **оба** вопроса (микрофон и команда).", ephemeral=True)
            return
...
async def send_application(interaction: discord.Interaction, data: dict):
    ...
    if channel is None:
        await interaction.followup.send("❌ Не удалось найти канал для заявок. Обратитесь к администратору.", ephemeral=True)
        return
    ...
    embed.add_field(name="🎮 Никнейм в игре", value=data["nickname"], inline=True)
    ...
    embed.add_field(name="🔨 Баны на проекте", value=data["bans"], inline=False)
    embed.add_field(name="🏆 1 Сезон / Достижения", value=data["season1"], inline=False)
    embed.add_field(name="📅 Планы на текущий сезон", value=data["plans_season"], inline=False)
    embed.add_field(name="⚔️ Деятельность на сервере", value=data["activities"], inline=False)
    embed.add_field(name=f"{mic_icon} Микрофон", value=data["microphone"].capitalize(), inline=True)
    embed.add_field(name=f"{team_icon} Игра в команде", value=data["team_ready"].capitalize(), inline=True)
    embed.add_field(name="\u200b", value="\u200b", inline=True)
    embed.add_field(name="🏘️ Поселения / Гильдии на сервере", value=data["previous_guilds"], inline=False)
    embed.add_field(name="🌐 Опыт в других проектах", value=data["other_projects"], inline=False)
    ...
    try:
        await user.send("✅ **Ваша заявка отправлена на рассмотрение!**\nМодераторы рассмотрят её в ближайшее время.")
        notify_text = "✅ Заявка отправлена! Уведомление также отправлено в личные сообщения."
    except discord.Forbidden:
        notify_text = "✅ Заявка отправлена на рассмотрение!\n_(Не удалось отправить ЛС — возможно, они закрыты.)_"
