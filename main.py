...
class AnketaModalPart2(Modal, title="Анкета 2/2"):
    plans_season = TextInput(label="6. Планы на текущий сезон", style=discord.TextStyle.paragraph, required=True, max_length=500)
    activities = TextInput(label="7. Чем займётесь на сервере?", style=discord.TextStyle.paragraph, required=True, max_length=300)
    previous_guilds = TextInput(label="9. Поселения/гильдии на сервере", style=discord.TextStyle.short, required=True, max_length=300)
    pvp_skill = TextInput(label="11. PvP-скилл (1-10)", style=discord.TextStyle.short, required=True, max_length=2)
    mechanics_skill = TextInput(label="12. Знание механик (1-10)", style=discord.TextStyle.short, required=True, max_length=2)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        data = {
            **self.part1_data,
            "plans_season": self.plans_season.value,
            "activities": self.activities.value,
            "previous_guilds": self.previous_guilds.value,
            "pvp_skill": pvp,
            "mechanics_skill": mechanics,
        }
        view = MicrophoneView(data)
        await interaction.response.send_message("🎙️ **Последний шаг!** Выберите ответы:", view=view, ephemeral=True)


class AnketaModalPart1(Modal, title="Анкета 1/2"):
    ...
    season1 = TextInput(label="5. Играли на 1 сезоне? Достижения?", style=discord.TextStyle.paragraph, required=True, max_length=500)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        part1_data = {
            ...
            "season1": self.season1.value,
        }
        await interaction.response.send_modal(AnketaModalPart2(part1_data))
...
class MicrophoneView(View):
    def __init__(self, data: dict):
        ...
        mic_select = Select(
            placeholder="8. Наличие микрофона",
            options=[
        ...
        team_select = Select(
            placeholder="13. Готовы играть в команде?",
            options=[
        ...
        submit_btn = Button(label="📨 Отправить анкету", style=discord.ButtonStyle.success, row=2)
...
async def send_application(interaction: discord.Interaction, data: dict):
    ...
    embed.add_field(name="🏘️ Гильдии", value=data["previous_guilds"], inline=False)
